...
import { FrameAiOutput, DaySummaryOutput, DaySummaryInput, ReflectionInput, ReflectionStoryOutput } from "./types";
import { buildDevelopFramePrompt, buildSummarizeDayPrompt } from "./prompts";
import { buildReflectPrompt } from "./reflection-prompts";
...
export async function reflectReal(
  ...
  const prompt = buildReflectPrompt(frames, spanDays);
  const { raw, model } = await callLlm(prompt);
  const parsed = parseJson<{
    title?: string;
    story: string;
  ...
  return {
    title: parsed.title,
    story: parsed.story || "镜子里暂时一片雾。",
    floatingWords: Array.isArray(parsed.floatingWords) ? parsed.floatingWords.slice(0, 16) : [],
    motifs: Array.isArray(parsed.motifs) ? parsed.motifs.slice(0, 6) : [],
    mood: Array.isArray(parsed.mood) ? parsed.mood.slice(0, 3) : [],
    basedOnCount: typeof parsed.basedOnCount === "number" ? parsed.basedOnCount : frames.length,
