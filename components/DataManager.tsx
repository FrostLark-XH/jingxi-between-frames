...
function validateBackupData(raw: unknown): {
  ...
  const migrated = migrateAll(raw);
  if (!migrated.success && migrated.frames.length === 0) {
    return { valid: false, error: migrated.error ?? "备份数据迁移失败" };
  }

  return { valid: true, frames: migrated.frames };
}
